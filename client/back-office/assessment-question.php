                ...
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <div class="d-flex flex-row-reverse bd-highlight">
                            <button class="btn btn-info text-white btn-previewAssessment" data-assessment_id="<?= $row['assessment_id'] ?>">Preview Assessment</button>
...
<div class="modal fade" id="assessmentPreviewModal" tabindex="-1" aria-labelledby="assessmentPreviewLabel">
  <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="assessmentPreviewLabel">Assessment Preview</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <div class="text-center mb-4">
          <h3 id="preview-assessment-title" class="fw-bold"></h3>
          <p id="preview-subject" class="text-muted"></p>
          <img id="preview-thumbnail" src="" alt="Assessment Thumbnail" class="img-fluid rounded mb-3" style="max-height: 250px;">
          <p id="preview-comment" class="text-secondary"></p>
        </div>
        <hr>
        <div id="preview-questions-list" class="text-start"></div>
      </div>
...
$(document).ready(function() {
  $(document).on('click', '.btn-previewAssessment', function() {
    const assessmentId = $(this).data('assessment_id');

    $.ajax({
      url: 'elements/popup/previewAssessments.php',
      type: 'GET',
      ...
      success: function(response) {
        try {
          ...
          // Fill in assessment info
          $('#preview-assessment-title').text(data.assessment_name);
          $('#preview-subject').text("Subject: " + (data.subject_name || "N/A"));
          $('#preview-thumbnail').attr('src', data.attach_file);
          $('#preview-comment').text(data.comment || "No description available.");
          ...
          if (data.questions && data.questions.length > 0) {
            data.questions.forEach((q, i) => {
              let questionImg = q.image_attachment 
                ? `<img src="${q.image_attachment}" alt="Question Image" class="img-fluid rounded mb-2" style="max-height:150px;">`
                : '';

              let choicesHTML = '';
              q.choices.forEach(choice => {
                const isCorrect = (choice.IsCorrectChoice == 0); // 0 = correct
                choicesHTML += `
              ...
              questionsHTML += `
                <div class="mb-4">
                  ...
                  <div class="mt-2">${questionImg}</div>
                  <ul class="list-group mt-2">${choicesHTML}</ul>
                </div>
              `;
            });
          } else {
            questionsHTML = '<p class="text-muted">No questions found for this assessment.</p>';
          }

          $('#preview-questions-list').html(questionsHTML);

          // Show modal
          const modal = new bootstrap.Modal(document.getElementById('assessmentPreviewModal'));
          modal.show();
      ...
      error: function() {
        alert('Error loading assessment preview.');
      }
    });
  });
});
