        ...
        function showPopup(event) {
            ...
            fetch('././backend/fetchAssessmentData.php?assessment_id=' + assessmentId)
                ...
                .then(assessmentData => {
                    ...
                    const imagePath = assessmentData.attach_file;
                    const imageElement = document.getElementById('currentImage');
                    imageElement.src = imagePath;
                    imageElement.style.display = imagePath ? "block" : "none";
            ...
            function showPopup(event) {
                ...
                fetch('././backend/fetchQuestionsData.php?question_id=' + questionId)
                    ...
                    .then(questionData => {
                        ...
                        //document.getElementById('questionImage').setAttribute('src', questionData.image_attachmnet);

                        const imageElement = document.getElementById('questionImage');

                        if (questionData.image_attachment && questionData.image_attachment.trim() !== "") {
                            // Build proper path (adjust base path as needed)
                            const imagePath = `${questionData.image_attachment}`;
                ...
                fetch('././backend/fetchChoicesData.php?question_id=' + questionId)
                    ...
                    .then(choicesData => {
                        ...
                        choicesData.forEach((choice, index) => {
                            ...
                            // IMPORTANT: DB uses 0 = correct, 1 = incorrect
                            const isCorrectChoice = String(choice.IsCorrectChoice) === '0';

                            // create checkbox (user-visible)
                            const checkbox = document.createElement('input');
                            checkbox.type = 'checkbox';
                            checkbox.name = `is_correct_choice[${index}]`; // user-visible only
                            checkbox.value = '0'; // when checked, we want to send '0' (correct)
                            checkbox.checked = isCorrectChoice;
                            checkbox.disabled = false;

                            // hidden input fallback (always sent). Will hold '0' or '1' per DB convention
                            const hiddenCorrect = document.createElement('input');
                            hiddenCorrect.type = 'hidden';
                            hiddenCorrect.name = `is_correct_choice[${index}]`; // same name to ensure single value per index
                            hiddenCorrect.value = isCorrectChoice ? '0' : '1';

                            // keep change handler to update hidden when checkbox toggles
                            checkbox.addEventListener('change', () => {
                                hiddenCorrect.value = checkbox.checked ? '0' : '1';
                            });

                            // text input (choice text)
                            ...
                            // hidden input for the choice id (so backend knows which DB row to update)
                            const hiddenId = document.createElement('input');
                            hiddenId.type = 'hidden';
                            hiddenId.name = `choice_id[${index}]`;
                            hiddenId.value = choice.choice_id || choice.choiceID || choice.choiceId; // adapt to returned key
                            ...
                            // append hidden inputs after visible controls
                            choiceContainer.appendChild(hiddenCorrect);
                            choiceContainer.appendChild(hiddenId);
                        ...
                        //     hiddenInput.name = `is_correct_choice[${index}]`;
                        //     hiddenInput.value = '0'; // default

                        //     checkbox.name = `is_correct_choice[${index}]`;
                        //     checkbox.value = '1'; // means "checked / correct"
                        ...
                        //     hiddenId.type = 'hidden';
                        //     hiddenId.name = `choice_id[${index}]`;
                        //     hiddenId.value = choice.choice_id;
                        //     choiceContainer.appendChild(hiddenId);
                        ...
                        //     inputText.value = choice.choiceText;
                        //     inputText.disabled = false; // Disable the text input
                            
                        //     inputGroupText.appendChild(checkbox, hiddenInput);
                        //     inputGroup.appendChild(inputGroupText);
                        //     inputGroup.appendChild(inputText);
                        //     choiceContainer.appendChild(inputGroup);
    ...
    <script>
        $(document).ready(function () {
            $("#question-details-popup").on("click", ".btn-primary", function () {
                var formData = new FormData($("#question-details-popup form")[0]);
                updateQuestion(formData);
            });

            function updateQuestion(formData) {
                $.ajax({
                    url: '././backend/edit-question.php',
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    success: function (response) {
                        if (response === 'success') {
                            ...
                            alert('Failed to update the question.' + response);
                            //location.reload();
                        }
                    },
                    error: function () {
                        ...
                        alert('Error occurred while updating the question.');
                        //location.reload();
                    }
                });
            }
